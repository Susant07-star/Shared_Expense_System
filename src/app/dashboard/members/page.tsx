...
import { Crown, User, Copy } from 'lucide-react'
  ...
  if (!membership) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        <p>You are not in any room yet.</p>
      </div>
    )
  }
        ...
        <section className="space-y-3">
          ...
          <div className="space-y-2">
            {admins.map((m: any) => {
              ...
              const joined = formatDate(m.joined_at, useNepali)
              return (
                <div
                  key={m.user_id}
                  className="flex items-center gap-4 p-4 rounded-xl border bg-card hover:bg-muted/30 transition-all duration-200"
                >
                  <Avatar name={name} />
